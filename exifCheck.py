...
def main():
    ...
    # 检查每个图片的EXIF信息
    images_with_exif = 0
    sample_count = len(image_files)  # 最多检查10个样本
    
    for i, image_path in enumerate(image_files[:sample_count]):
    ...
    # 统计信息
    print("=" * 50)
    print(f"检查了 {sample_count} 个样本文件")
    print(f"其中 {images_with_exif} 个文件包含EXIF信息")
    
    if images_with_exif > 0:
        print("\n建议: 您的图片包含EXIF信息，建议在压缩时移除以保护隐私")
    else:
        print("\n建议: 您的图片不包含EXIF信息，无需特别处理")
